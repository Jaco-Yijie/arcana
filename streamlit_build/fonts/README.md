...
三份 WOFF2，总计约 119.0 KiB，保留现有 OFL 授权；`font-display: swap`，无外部字体请求。

| 字体 | 用途 | 大小 |
|---|---|---:|
| Cinzel | 品牌、英文辅助铭文 | 13.9 KiB |
| Cormorant Garamond | 拉丁动态标题、仪式按钮回退 | 36.9 KiB |
| LXGW WenKai Light 子集 | 中文固定标题、章节、引导语、主要按钮 | 68.2 KiB |
...
子集字符由语言资源和 `src/i18n/display-keys.json` 推导，清单见 `subset-charset.txt`。扩充静态文案时更新 `scripts/subset-display-font.py` 并运行：
...
动态核心主题使用 `--font-oracle-dynamic`：Cinzel + 完整系统楷体/宋体，不含中文子集。英文启动仅请求两份拉丁字体；中文文楷由 boot.ts 按需注入。
