 ...
 * （示例正文 2598 字符 → 模型输出 2968 字符）。
 * 也就是说**示例本身就是最强的篇幅锚点** —— 光在文字里写「要精炼」
 * 敌不过一份摆在眼前的长样例，实测只降了 16%。
 *
 * 所以标准模式换一份按它自己的预算写的示例：同样的牌、同样的 json 形状，
 * 但每个字段都是标准模式该有的密度，且不含 alternativeInterpretations。
 * 深度模式继续用上面那份完整的。
 */
...
const OUTPUT_EXAMPLE_STANDARD_SECTION = `# 输出示例（只演示 json 形状与篇幅密度，内容与本次无关）
